...
class FlashManager:
    def __init__(self, root):
        self.root = root
        self.root.title("Flash Manager - Smart File Transfer")
        
        # Fullscreen mode IMMEDIATELY AT STARTUP
        ...
        # Determine source folder (where the program is located)
    ...
    # === HARD EXIT (NEW VERSION 0.9) ===
    
    def hard_exit(self):
        """Instant program shutdown without errors or reports to Microsoft"""
        try:
            self.root.quit()           # Stop mainloop
            self.root.update_idletasks() # Force process all events
            self.root.destroy()        # Destroy the window
        except:
            pass
        os._exit(0)                    # Full process exit (guaranteed!)
    ...
    def add_current_path_to_saved(self):
        ...
        if not current:
            messagebox.showwarning("Warning", "Path field is empty")
            return
        
        for item in self.saved_paths:
            if item["path"] == current:
                messagebox.showinfo("Information", "This path is already in the list")
                return
    ...
    def show_dropdown(self, event=None):
        ...
        if not self.saved_paths:
            messagebox.showinfo("List empty", "No saved paths.\nClick ➕ to add.")
            return
        ...
        tk.Label(header, text="Saved paths", bg="#222222", fg=self.fg_color,
    ...
    def get_drive_info(self):
        ...
        try:
            ...
            if "C:" in drive.upper():
                info["type"] = "SSD / HDD (system)"
            elif any(d in drive.upper() for d in ["D:", "E:", "F:", "G:"]):
                info["type"] = "External drive / USB flash"
            else:
                info["type"] = "Local disk"
        except:
            pass
        return info
    ...
    def update_right_panel(self):
        ...
        tk.Label(chart_frame, text="📊 Item sizes in folder", bg=self.bg_color, fg=self.fg_color,
        ...
        if total_size > 0:
            ...
            if others_size > 0:
                ...
                color_map.append(("Others", "#666666", False))
            ...
            for i, (name, color, is_dir) in enumerate(color_map[:5]):
                ...
                size = next((s for n, s, d in top_items if n == name), 0) if name != "Others" else others_size
                if size > 0:
            ...
            tk.Label(chart_frame, text="Folder is empty", bg=self.bg_color, fg="#888", font=("Segoe UI", 10)).pack(pady=20)
        
        for widget in self.right_panel.winfo_children():
            widget.destroy()
        info_frame = tk.Frame(self.right_panel, bg=self.bg_color)
        info_frame.pack(fill=tk.X, pady=10)
        tk.Label(info_frame, text="💾 DRIVE", bg=self.bg_color, fg=self.fg_color,
                 font=("Segoe UI", 14, "bold")).pack()
        font_large = ("Segoe UI", 12)
        info_lines = [
            f"Drive: {self.drive_info['drive']}",
            f"Type: {self.drive_info['type']}",
            f"FS: {self.drive_info['fs']}",
            f"Status: {self.drive_info['status']}",
            "",
            f"Total: {self.format_size(self.drive_info['total'])}",
            f"Used: {self.format_size(self.drive_info['used'])}",
            f"Free: {self.format_size(self.drive_info['free'])}",
            f"Used: {self.drive_info['percent']:.1f}%"
        ]
    ...
    def create_back_button(self, parent_frame, state="normal"):
        ...
        text_id = canvas.create_text(40, 15, text="← Back", fill=self.btn_fg, font=("Segoe UI", 10), tags="text")
        if state == "normal":
    ...
    def setup_ui(self):
        ...
        # Fullscreen button (FIRST ON THE LEFT, no background)
        ...
        # Other elements
        tk.Label(top_line, text="📂 Copy to:", font=("Segoe UI", 10), bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(0,5))
        ...
        # ========== PANEL WITH BACK BUTTON AND SEARCH ==========
        ...
        tk.Label(search_frame, text="Search drive:", font=("Segoe UI", 10), bg=self.bg_color, fg=self.fg_color).pack(side=tk.LEFT, padx=(0,5))
        ...
        self.search_btn = self.create_cut_corner_button(search_frame, text="SEARCH", command=self.start_search, width=8)
        ...
        # ========== EXIT BUTTON (VERSION 0.9 — HARD SHUTDOWN) ==========
        exit_btn = self.create_cut_corner_button(right_bottom, text="Exit", command=self.hard_exit, width=6, bg=self.btn_exit_bg)
    ...
    def _is_child_of_left_panel(self, widget, depth=0):
        if depth > 100:  # Protection against recursion
            return False
        if widget is None:
            return False
    ...
    def show_suggestions(self):
        ...
        try:
            ...
            self.suggestion_window.title("Suggestions")
            ...
            header.pack(fill=tk.X)
            tk.Label(header, text="Select folder:", bg="#2b2b2b", fg="white").pack(side=tk.LEFT, padx=5, pady=2)
    ...
    def refresh_file_list(self):
        ...
        if self.is_searching and self.search_results:
            ...
            try:
                ...
                return
            if not items:
                tk.Label(self.scrollable_frame, text="This folder is empty", bg=self.bg_color, fg="#888888", font=("Segoe UI", 12)).pack(expand=True, pady=50)
            else:
    ...
    def create_item_row(self, item):
        ...
        copy_btn = self.create_fixed_button(buttons_frame, "Copy", lambda p=item: self.ask_copy(p), width=90)
        copy_btn.pack(side=tk.RIGHT, padx=2)
        if not item.is_dir() and item.suffix.lower() in ['.zip', '.rar']:
            extract_btn = self.create_fixed_button(buttons_frame, "Extract", lambda p=item: self.ask_extract(p), width=90)
    ...
    def ask_destination(self, item, action):
        ...
        tk.Label(dialog, text=f"Where to {action == 'extract' and 'extract' or 'copy'} '{item.name}'?",
        ...
        btn1 = self.create_cut_small_button(btn_frame, "📂 To the folder specified above", use_default, width=28)
        btn1.pack(pady=5)
        btn2 = self.create_cut_small_button(btn_frame, "🔍 Choose manually", choose_manually, width=28)
        btn2.pack(pady=5)
        btn3 = self.create_cut_small_button(btn_frame, "❌ Cancel", dialog.destroy, width=28)
        btn3.pack(pady=5)
    ...
    def execute_action(self, item, action, destination):
        try:
            ...
            if action == "copy":
                ...
                messagebox.showinfo("Success", f"✅ Copied: {item.name}")
            elif action == "extract":
                if item.suffix.lower() == '.zip':
                    with zipfile.ZipFile(item, 'r') as zip_ref:
                        zip_ref.extractall(destination)
                    messagebox.showinfo("Success", f"✅ Extracted ZIP: {item.name}")
                elif item.suffix.lower() == '.rar':
                    messagebox.showinfo("RAR", f"File {item.name} copied as is.\nTo extract RAR, WinRAR or 7-Zip is required.")
                    shutil.copy2(item, destination)
                else:
                    messagebox.showerror("Error", "File is not an archive (.zip or .rar)")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to perform action:\n{str(e)}")
